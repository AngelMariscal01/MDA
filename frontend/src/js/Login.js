      ...
      axios
        ...
        .then((response) => {
          ...
          if (token) {
            ...
            if (
              parseJwt(token).rol === "admin" &&
              parseJwt(token).estado === "activo"
            ) {
              toast.success("Bienvenido Administrador!");
              setTimeout(() => {
                navigate("/inicioAdministrador", {
                  state: {
                    usuarioId: parseJwt(token).usuario_id,
                    rol: parseJwt(token).rol,
                  },
                });
                window.location.reload();
              }, 1000);
            } else if (
              parseJwt(token).rol === "cliente" &&
              parseJwt(token).estado === "activo"
            ) {
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-8">
        <div
          className="rounded-xl bg-cover bg-center bg-no-repeat min-h-[200px] sm:min-h-[250px] md:min-h-[300px] lg:min-h-[350px] flex flex-col justify-end glow-effect"
          style={{
      ...
      <a
        ...
        className="text-[#D97789] text-sm text-center underline pb-3 sm:text-base"
      >
        ¿Olvidaste tu Contraseña?
      </a>
      <a
